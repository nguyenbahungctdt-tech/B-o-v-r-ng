...
struct KeyValidationResult {
    ...
    func monitorKeyChanges(key: String, onStatusChanged: @escaping (Bool, String, String) -> Void) {
        ...
        keyListenerHandle = dbRef?.child("ActivationKeys").child(key).observe(.value) { snapshot in
            ...
            let dateStr = df.string(from: Date(timeIntervalSince1970: TimeInterval(expiry / 1000)))
    ...
    func syncPolygon(user: [String: String], p: [String: Any], cm: Double, province: String, zone: Int) async -> Bool {
        ...
        let dateStr = df.string(from: Date(timeIntervalSince1970: TimeInterval(timestamp / 1000)))
...
class CloudSyncRepository {
    ...
    func uploadSurveyPhoto(localPath: String, remotePath: String) async -> String? {
        ...
        do {
            ...
            if !active { return KeyValidationResult(isValid: false, message: "Key bị khóa!")     private var keyListenerHandle: DatabaseHandle?
    ...
    func uploadSurveyPhoto(localPath: String, remotePath: String) async -> String? {
        ...
        do {
            ...
            if let info = userInfo {
                ...
                if !rName.isEmpty && rName != info["name"] { return KeyValidationResult(isValid: false, message: "Họ tên không khớp!")     private var keyListenerHandle: DatabaseHandle?
    ...
    func uploadSurveyPhoto(localPath: String, remotePath: String) async -> String? {
        ...
        do {
            ...
            return nil
                ...
                if !rEmail.isEmpty && rEmail != info["email"] { return KeyValidationResult(isValid: false, message: "Email không khớp!")     private var keyListenerHandle: DatabaseHandle?
    ...
    func syncPatrol(user: [String: String], patrol: [String: Any], cm: Double, province: String, zone: Int) async -> Bool {
        guard let unit = user["unit"], let name = user["name"] else { return false     private var keyListenerHandle: DatabaseHandle?
    ...
    func updatePersonnelInfo(user: [String: String], registrationKey: String, permissions: String, canSync: Bool) async {
        guard let name = user["name"] else { return     private var keyListenerHandle: DatabaseHandle?
    ...
    func uploadSurveyPhoto(localPath: String, remotePath: String) async -> String? {
        ...
        let df = DateFormatter()
        df.dateFormat = "yyyy-MM-dd"
        let dateStr = df.string(from: Date(timeIntervalSince1970: TimeInterval(startTime / 1000)))
    ...
    func syncDailyJournal(user: [String: String], journal: [String: Any], cm: Double, province: String, zone: Int) async -> Bool {
        guard let unit = user["unit"], let name = user["name"], let dateStr = journal["date"] as? String else { return false     private var keyListenerHandle: DatabaseHandle?
